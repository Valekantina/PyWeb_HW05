...
async def get_data(date):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            print(
                f"Error getting data for date {date.strftime('%d.%m.%Y')}: {err}")
            return None
        ...
        for rate in d['exchangeRate']:
            ...
            if rate['currency'] in currencies:
                if 'saleRate' in rate and 'purchaseRate' in rate:
                    item[rate['currency']] = {
                        'sale': rate['saleRate'], 'purchase': rate['purchaseRate']}
                else:
                    continue
        ...
        for currency in currencies:
            if currency in item:
                result_item[item['date']][currency] = {
                    'sale': item[currency]['sale'], 'purchase': item[currency]['purchase']}
